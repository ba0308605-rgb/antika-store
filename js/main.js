        ...
        if (dropdown) {
            dropdown.innerHTML = categories.map(cat => `
                <a href="products.html?category=${cat.id}" class="block px-4 py-2 hover:bg-antika-lavender transition text-gray-700">
                    ${cat.icon && (cat.icon.startsWith('http') || cat.icon.startsWith('data:')) ? `<img src="${cat.icon}" style="width:26px;height:26px;object-fit:contain;vertical-align:middle;display:inline-block">` : cat.icon} ${cat.name}
                </a>
            `).join('');
        }
            ...
            grid.innerHTML = categories.map(cat => `
                <a href="products.html?category=${cat.id}" class="group">
                    <div class="bg-antika-lavender rounded-2xl p-6 text-center hover:shadow-xl transition transform hover:-translate-y-2 border-2 border-transparent hover:border-antika-pink">
                        <div class="text-5xl mb-4 group-hover:scale-110 transition">${cat.icon && (cat.icon.startsWith('http') || cat.icon.startsWith('data:')) ? `<img src="${cat.icon}" style="width:72px;height:72px;object-fit:contain;display:inline-block">` : cat.icon}</div>
            ...
            if (categories.length === 0) {
                ...
                mobileCategories.innerHTML = categories.map(cat => `
                    <a href="products.html?category=${cat.id}" class="flex items-center gap-3 py-2 text-gray-700 hover:text-antika-pink transition" onclick="toggleMenu()">
                        <span class="text-xl">${cat.icon && (cat.icon.startsWith('http') || cat.icon.startsWith('data:')) ? `<img src="${cat.icon}" style="width:30px;height:30px;object-fit:contain;vertical-align:middle">` : (cat.icon || '📦')}</span>
                        <span>${cat.name}</span>
                    </a>
                `).join('');
            }
