...
app.put('/api/notifications/read-all', async (req, res) => {
  try {
    const { email } = req.body || {};
    if (!email) return res.status(400).json({ error: '\u0627\u0644\u0625\u064a\u0645\u064a\u0644 \u0645\u0637\u0644\u0648\u0628' });
    const s = await db.collection('notifications').where('ownerEmail', '==', email).where('read', '==', false).get();
    const b = db.batch(); s.docs.forEach(d => b.update(d.ref, { read: true, newLikes: 0 })); await b.commit();
    res.json({ success: true });
  } catch (err) { res.status(500).json({ error: err.message }); }
});
